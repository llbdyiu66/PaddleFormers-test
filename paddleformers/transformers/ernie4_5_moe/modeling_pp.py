...
class Ernie4_5_MoeEmbeddingPipe(nn.Layer):
    """Extends Ernie4_5_MoeEmbeddingPipe to forward attention_mask through the pipeline."""
    ...
    def __init__(self, config):
        ...
        super(Ernie4_5_MoeEmbeddingPipe, self).__init__()
...
class MTPEmbeddingPipe(Ernie4_5_MoeEmbeddingPipe):
    """Extends Ernie4_5_MoeEmbeddingPipe to forward attention_mask through the pipeline."""
...
class Ernie4_5_MoeDecoderLayerPipe(Ernie4_5_MoeDecoderLayer):
    """
...
class MTPLayer(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.mtp_block = paddle.nn.LayerList(
            [Ernie4_5_MoeDecoderLayer(config, layer_idx) for layer_idx in range(self.config.num_nextn_predict_layers)]
        )
...
class Ernie4_5_MoeForCausalLMPipe(PipelinePretrainedModel, PipelineLayer):
    ...
    _init_weights = Ernie4_5_MoePretrainedModel._init_weights
    _keep_in_fp32_modules = Ernie4_5_MoePretrainedModel._keep_in_fp32_modules
    ...
    def __init__(
        ...
        if config.tie_word_embeddings:
            ...
            self.add_sequential_layer(LayerDesc(Ernie4_5_MoeEmbeddingPipe, config=config), "ernie")

        for i in range(config.num_hidden_layers):
            self.add_sequential_layer(
                LayerDesc(
                    Ernie4_5_MoeDecoderLayerPipe,
        ...
        seg_method = (
            ...
            else "layer:Ernie4_5_MoeDecoderLayer|EmptyLayer|MTPLayer"
        )
        try:
        ...
        if (
            seg_method == "layer:Ernie4_5_MoeDecoderLayer|EmptyLayer"
